...
Response *Response_new() {
  Response *resp = (Response *)emalloc(sizeof(Response));
  resp->headers = (char *)ecalloc(sizeof(char), RESPONSE_HEADERS_MAX_SIZE);
  resp->headers_size = 0;
...
Url *Url_new() {
  ...
  url->host = (char *)ecalloc(sizeof(char), HOST_MAX_LENGTH);
  url->path = (char *)ecalloc(sizeof(char), PATH_MAX_LENGTH);
...
void read_data(int sock_fd, Response *resp) {
  ...
  if (start_of_body) {
    ...
    free(resp->body);
    resp->body = (char *)ecalloc(sizeof(char), resp->body_size + 10);
...
char *create_request(Url *url) {
  char *request = (char *)ecalloc(sizeof(char), REQUEST_MAX_SIZE);
...
void make_request(int sock_fd, Url *url) {
  ...
  if (written_size < 0) {
    err_with_close("error on writing to socket", sock_fd);
  }
...
int main(int argc, char **argv) {
  ...
  while (!scrolled_to_end) {
    ...
    if (poll_result < 0) {
      err_with_close("error on poll", sock_fd);
    }
